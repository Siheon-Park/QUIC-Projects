...
if __name__ == '__main__':
    ...
    stepsize = 0.001
    n_feature = 10
    n_qubits_list = np.arange(6, 11+1, dtype=int) # 6 7 8 9 10 11
    n_layers_list = np.arange(1, 9+1, 1, dtype=int) # 1 3 5 7 9
    C=10**3
    lamda=10**3
    dir_name = 'QAA_simulation_2'
...
if __name__ == '__main__':
    ...
    dir_path = Path('/home/quic/QUIC-Projects/summary_ipynb/2023/01 January/06 Fri') / dir_name
...
def update_csv(dir_path:Path, n_qubits_list, n_layers_list):
    ...
    for n_qubits, n_layers in product(n_qubits_list, n_layers_list):
        ...
        if sub_sub_dir_path.joinpath('grad_result.json').exists():
            with open(sub_sub_dir_path/'grad_result.json', 'r') as fp:
                grad_result = json.load(fp)
            grad_exp = grad_result['grad_exp']
            grad_var = grad_result['grad_var']
...
def training(args):
    ...
    if not sub_sub_dir_path.joinpath('result.json').exists():
        ...
        #         # oracle-like layer
        #         qml.BasicEntanglerLayers(layer_params.reshape(1, -1), wires=device.wires, rotation=qml.RY)
        #         # quantum diffusion layer
        #         qml.GroverOperator(wires=device.wires)
        def var_form(params):
            # initial state |+>^n
            for w in device.wires:
                qml.Hadamard(wires=w)
...
def gradient_analysis(args):
    ...
    device:qml.Device = qml.device('lightning.qubit', wires=n_qubits) # TODO: gpu vs cpu
    sub_sub_dir_path = sub_dir_path / f'n_layers={n_layers}'
    if not sub_sub_dir_path.joinpath('grad_result.json').exists():
        # data loading
        with open(sub_sub_dir_path/'reference.json', 'r') as fp:
            reference = json.load(fp=fp)
        reproducible_seed = reference['seed']
        ...
        grad_results_00 = np.empty(shape=(n_samples,))
        # grad_fn = qml.jacobian(qasvm.cost_fn)
        # grad_fn = qml.jacobian(qasvm.avg_f)
        for s in range(n_samples):
            params=qml.numpy.random.random(parameter_shape, requires_grad=True)
            grad_results_00[s] = qasvm.grad_fn_00(params)
        ...
        # save result
        with open(sub_sub_dir_path/'grad_result.json', 'w') as fp:
            json.dump(dict(
                grad_exp=grad_exp, grad_var=grad_var, grad_results_00=grad_results_00, sample_size=n_samples
            ), fp=fp, default=list)
...
if __name__=='__main__':
    ...
    with mp.Pool(mp.cpu_count()) as pool:
        ...
        # pool.map(gradient_analysis, product(n_qubits_list, n_layers_list))
    update_csv(dir_path, n_qubits_list, n_layers_list)