...
Label = List[float]
Data = List[float]
Datadict = Dict[Any, Data] # key: class, value: data

class Swap_classifier(Instruction):
    def __init__(self, datadict:Datadict, weight:Optional=None, testdata:Optional=None):
        """
        ...
        dataset = np.vstack(list(datadict.values()))
        label = []
        for l in labels:
            for i in range(len(datadict[l])):
                label.append(l)
