...
hinge = lambda x, t: np.maximum(0, 1-x*t)
...
class Classifier:
    ...
    def plot(self, option:str='sv', axes=plt, **kwargs):
        ...
        if 'data' in option:
            ...
            support_vector = data[self.alpha>=1e-10]
            axes.scatter(data[:,a[0]], data[:,a[1]], c=self.label, cmap=cmap)
...
class SVM(Classifier):
    ...
    def optimize(self, initial_point:np.ndarray, method:str='SLSQP', validation_data:np.ndarray=None, validation_label:np.ndarray=None, mimic='', **options):
        if mimic =='qsvm':
            ...
            cnts = {'type':'eq', 'fun':lambda x:(1-np.sum(x))**2}
            ...
            bnds = tuple([(0, 1) for i in range(self.num_data)])
            cnts = {'type':'eq', 'fun':lambda x:(1-np.sum(x))**2}
            ret = sp.optimize.minimize(self.empirical_loss, initial_point, method=method, bounds = bnds, constraints=cnts, options=options)
        elif mimic == 'qusvm':
            ret = sp.optimize.OptimizeResult()
            ret.message = 'Uniform weight estimation'
            ret.x = np.ones(self.num_data)/self.num_data
        else:
            bnds = tuple([(0, 1) for i in range(self.num_data)])
            ret = sp.optimize.minimize(self.objective_function, initial_point, method=method, bounds = bnds, options=options)
        self.opt_result = ret
        self.alpha = ret.x

    def objective_function(self, alpha:np.ndarray):
        ret = 0.5*(self.C)*(self.ZZZval(alpha)+self.IZZval(alpha))-np.sum(alpha)
        return ret

    def objective_ffunction(self, alpha:np.ndarray):
        ret = 0.5*(self.ZZZval(alpha)+self.IZZval(alpha))
        return ret

    '''
        def empirical_loss(self, alpha:np.ndarray):
            q = sigmoid(self.C*(self.ZZval(alpha, self.validation_data)+self.IZval(alpha))).reshape(-1)
            return np.sum(H_cbin((self.validation_label+1)/2, q))/len(self.validation_label)
    '''
    def empirical_loss(self, alpha:np.ndarray):
        q = (self.ZZval(alpha, self.validation_data)+self.IZval(alpha)).reshape(-1)
        return self.C*np.sum(hinge(self.validation_label, q)) + self.objective_ffunction(alpha)
    ...
    def predict(self, test:np.ndarray, alpha:np.ndarray=None):
        if not isinstance(alpha, np.ndarray):
            alpha = self.alpha
        est_y = np.sign( self.ZZval(alpha, test)+self.IZval(alpha) )
        return est_y

    def accuracy(self, test:np.ndarray, testlabel:np.ndarray, alpha:np.ndarray=None):
        if not isinstance(alpha, np.ndarray):
            alpha = self.alpha
