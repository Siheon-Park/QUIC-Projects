...
def get_loggers_add_handle(*objects, level: int = logging.DEBUG, handle: logging.Handler = None,
                           form: logging.Formatter = None, **kwargs):
    ...
    for obj in objects:
        ...
        if not isinstance(handle, logging.Handler):
            filepath = kwargs.get('filename', DEFAULT_EXP_LOG_PATH / (
                        time.strftime('%y%m%d-%H%M%S-', time.localtime(time.time())) + str(uuid.uuid4()) + '.log'))
