...
# noinspection PyShadowingBuiltins,SpellCheckingInspection,PyAttributeOutsideInit
class QASVM(QuantumClassifier):
    ...
    def __init__(self,
                 ...
                 C: float = 1, k: float = 0.1, option: Union[str, Any] = 'Bloch_sphere') -> None:
    ...
    def set_result(self, optimal_parameters: Union[np.ndarray, dict], opt_val: float = None,
                   num_optimizer_evals: int = 0, eval_time: float = 0):
    ...
    def _construct_firt_order_circuit(self):
        ...
        qc = self.circuit_class(self.num_data, self.dim_data, order=1)
        qc.add_var_form(self.var_form)
        qc.UD_encode(self.feature_map, self.feature_map_params, training_data=self.data, training_label=self.label,
                     N=self.num_data)
    ...
    def _construct_second_order_circuit(self):
        ...
        parameters """
        qc = self.circuit_class(self.num_data, self.dim_data, order=2)
        if self.var_form is not None:
            qc.add_var_form(
                self.var_form.assign_parameters(dict(zip(self.optimization_params, self.var_form_params['i']))),
                reg='i')
            qc.add_var_form(
                self.var_form.assign_parameters(dict(zip(self.optimization_params, self.var_form_params['j']))),
                reg='j')
        else:
    ...
    def _evaluate_first_order_circuit(self, theta: Union[np.ndarray, List[float], dict],
