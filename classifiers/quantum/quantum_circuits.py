...
class QASVM_circuit(QuantumCircuit):
    ...
    def __init__(self, num_data: int, dim_data: int, order: int) -> None:
        assert (order == 1 or order == 2)
        super().__init__()
        regs = [QuantumRegister(1, 'a'), QuantumRegister(int(math.log2(num_data)), 'i'),
                QuantumRegister(int(math.log2(dim_data)), 'xi'), QuantumRegister(1, 'yi')]
        if order == 1:
    ...
    def X_encode(self, feature_map, feature_map_params, testdata: Union[np.ndarray, ParameterVector, List[Parameter]],
                 reg: str = 'xj'):
